...
- **CVE pins:** the root `build.gradle.kts` force-resolves patched versions of 6 transitive deps with known CVEs (jose4j, jdom2, bcprov/bcpkix, commons-lang3, httpclient). Do not remove these `resolutionStrategy.force` entries when touching dependencies — Dependabot can't update transitive deps.
